...
The embedded transport honors its validated connection configuration. Direct SSH keeps the normal argument vector unchanged. Tufts VPN mode adds command-line overrides for `HostName=127.0.0.1`, the private local port, `HostKeyAlias=login-prod.pax.tufts.edu`, `ProxyCommand=none`, and `ProxyJump=none` before the destination. This preserves known-host verification for the real server while preventing a configured proxy from bypassing the local endpoint.
